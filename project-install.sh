...
# Cross-platform script to configure telemetry environment variables in Claude Code project settings
...
# Function to get Claude Code project settings directory
get_settings_dir() {
    echo "$(pwd)/.claude"
}
...
set_env_variables() {
    ...
    # Build OTEL_RESOURCE_ATTRIBUTES with project information
    local base_attributes="$DEFAULT_OTEL_RESOURCE_ATTRIBUTES"
    if [[ -n "$CLI_OVERRIDE_RESOURCE_ATTRIBUTES" ]]; then
        base_attributes="$CLI_OVERRIDE_RESOURCE_ATTRIBUTES"
    fi
    
    # Add project to resource attributes
    if [[ -n "$base_attributes" ]]; then
        export OTEL_RESOURCE_ATTRIBUTES="$base_attributes,project=$CLI_OVERRIDE_PROJECT"
    else
        export OTEL_RESOURCE_ATTRIBUTES="project=$CLI_OVERRIDE_PROJECT"
    fi
    ...
    echo "Project: $CLI_OVERRIDE_PROJECT (added to OTEL_RESOURCE_ATTRIBUTES)"
}

# Function to create project .claude/settings.json with proper structure
create_project_settings() {
    ...
    if [[ -f "$settings_file" ]]; then
        ...
        # Create new .claude/settings.json with all telemetry environment variables
        cat > "$temp_file" << EOF
{
  "env": {
...
parse_arguments() {
    ...
    # Validate that required --project flag was provided
    if [[ -z "$CLI_OVERRIDE_PROJECT" ]]; then
        echo "Error: --project flag is required" >&2
        echo "Use --help to see available options" >&2
        exit 1
    fi
}
...
main() {
    ...
    echo "Claude Code Project Environment Variable Merger"
    echo "==============================================="
    ...
    # Get Claude Code project settings directory
    local settings_dir
    settings_dir=$(get_settings_dir)
    echo "Project settings directory: $settings_dir"
    
    # Create settings directory if it doesn't exist
    if [[ ! -d "$settings_dir" ]]; then
        echo "Creating project settings directory: $settings_dir"
        mkdir -p "$settings_dir"
    fi
    
    # Path to settings.json
    local settings_file="$settings_dir/settings.json"
    echo "Project settings file: $settings_file"
    
    # Create or update project settings.json
    create_project_settings "$settings_file"
    
    # Display current settings
    display_settings "$settings_file"
    
    echo ""
    echo "✅ Successfully merged telemetry environment variables into project Claude Code settings!"
    echo ""
    echo "The following environment variables are now available to Claude Code for this project:"
    ...
    echo "Project-specific telemetry is now configured for: $CLI_OVERRIDE_PROJECT"
}

# Check for help flag
if [[ "${1:-}" == "--help" ]] || [[ "${1:-}" == "-h" ]]; then
    echo "Claude Code Project Telemetry Environment Variable Merger"
    echo ""
    echo "This script configures OpenTelemetry environment variables in Claude Code's project settings file."
    echo ""
    echo "Usage: $0 --project <name> [OPTIONS]"
    echo ""
    echo "REQUIRED OPTIONS:"
    echo "  --project <name>                Set project name (required, added to OTEL_RESOURCE_ATTRIBUTES)"
    echo ""
    ...
    echo "  --resource-attributes <attrs>   Set OTEL_RESOURCE_ATTRIBUTES (project will be appended)"
    echo "  -h, --help                      Show this help message"
    echo ""
    echo "Examples:"
    echo "  $0 --project my-webapp                                     # Use defaults with project name"
    echo "  $0 --project api-service --endpoint http://otel.company.com:4317  # Override endpoint"
    echo "  $0 --project dev-tool --service-name dev-team --protocol http     # Multiple overrides"
    echo ""
    ...
    echo "  - Adds project name to OTEL_RESOURCE_ATTRIBUTES"
    echo "  - Detects your operating system (Linux, macOS, Windows/WSL)"
    echo "  - Creates or updates .claude/settings.json in the current project directory"
    echo "  - Preserves existing project settings while adding telemetry configuration"
    echo ""
    ...
    echo "  - OTEL_RESOURCE_ATTRIBUTES       Resource attributes (project will be automatically added)"
